...
// -------
//
function ciniki_lapt_documentUpdate(&$ciniki) {
    //
    ...
    $rc = ciniki_core_prepareArgs($ciniki, 'no', array(
        ...
        'image_id'=>array('required'=>'no', 'blank'=>'no', 'name'=>'Primary Image'),
    ...
    $rc = ciniki_lapt_checkAccess($ciniki, $args['tnid'], 'ciniki.lapt.documentUpdate');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['title']) ) {
        ...
        if( $rc['num_rows'] > 0 ) {
             return array('stat'=>'fail', 'err'=>array('code'=>'ciniki.lapt.10', 'msg'=>'You already have an document with this title, please choose another.'));
        }
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.lapt.document', $args['document_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($args['types']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.lapt', 'tag', $args['tnid'],
            ...
            'document_id', $args['document_id'], 20, $args['types']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.lapt');
            return $rc;
        }
    ...
    if( isset($args['categories']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.lapt', 'tag', $args['tnid'],
            'ciniki_lapt_tags', 'ciniki_lapt_history',
            'document_id', $args['document_id'], 40, $args['categories']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.lapt');
            return $rc;
        }
    ...
    if( isset($args['tags']) ) {
        ...
        $rc = ciniki_core_tagsUpdate($ciniki, 'ciniki.lapt', 'tag', $args['tnid'],
            'ciniki_lapt_tags', 'ciniki_lapt_history',
            'document_id', $args['document_id'], 40, $args['tags']);
        if( $rc['stat'] != 'ok' ) {
            ciniki_core_dbTransactionRollback($ciniki, 'ciniki.lapt');
            return $rc;
        }
    ...
    ciniki_core_hookExec($ciniki, $args['tnid'], 'ciniki', 'web', 'indexObject', array('object'=>'ciniki.lapt.document', 'object_id'=>$args['document_id']));
